...
ProfileEventSingleton::instance().makeQuantityEvent( R"(@n-nonrecursive-relation;eql;hack.dl [10:7-10:10];)",rel_eql_e1be027843ac9bc5->size(),iter);ProfileEventSingleton::instance().makeQuantityEvent( R"(@n-nonrecursive-relation;ids;hack.dl [61:7-61:10];)",rel_ids_91388717b796d22d->size(),iter);ProfileEventSingleton::instance().makeQuantityEvent( R"(@n-nonrecursive-relation;leader;hack.dl [60:7-60:13];)",rel_leader_d565398e1ef92697->size(),iter);{
...
in file hack.dl [68:1-70:44])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;leader;hack.dl [68:1-70:44];leader(x,y1) <= leader(x,y2) :- \n   eql(y1,y2),\n   y2 = as(@eqfind(as(y1, number)), Id).;)_",iter, [&](){return rel_leader_d565398e1ef92697->size();});
...
{
	Logger logger(R"_(@t-recursive-relation;ids;hack.dl [61:7-61:10];)_",iter, [&](){return rel_new_ids_5220eba57eff354c->size();});
signalHandler->setMsg(R"_(ids(a) :- 
   add(a,_,_).
in file hack.dl [62:1-62:24])_");
{
	Logger logger(R"_(@t-recursive-rule;ids;0;hack.dl [62:1-62:24];ids(a) :- \n   add(a,_,_).;)_",iter, [&](){return rel_new_ids_5220eba57eff354c->size();});
if(!(rel_delta_add_241d705dc2fb23df->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_add_241d705dc2fb23df_op_ctxt,rel_delta_add_241d705dc2fb23df->createContext());
...
CREATE_OP_CONTEXT(rel_ids_91388717b796d22d_op_ctxt,rel_ids_91388717b796d22d->createContext());
for(const auto& env0 : *rel_delta_add_241d705dc2fb23df) {
if( !(rel_ids_91388717b796d22d->contains(Tuple<RamDomain,1>{{ramBitCast(env0[0])}},READ_OP_CONTEXT(rel_ids_91388717b796d22d_op_ctxt)))) {
Tuple<RamDomain,1> tuple{{ramBitCast(env0[0])}};
...
signalHandler->setMsg(R"_(ids(a) :- 
   add(_,a,_).
in file hack.dl [63:1-63:24])_");
{
	Logger logger(R"_(@t-recursive-rule;ids;0;hack.dl [63:1-63:24];ids(a) :- \n   add(_,a,_).;)_",iter, [&](){return rel_new_ids_5220eba57eff354c->size();});
if(!(rel_delta_add_241d705dc2fb23df->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_add_241d705dc2fb23df_op_ctxt,rel_delta_add_241d705dc2fb23df->createContext());
CREATE_OP_CONTEXT(rel_new_ids_5220eba57eff354c_op_ctxt,rel_new_ids_5220eba57eff354c->createContext());
...
signalHandler->setMsg(R"_(ids(a) :- 
   add(_,_,a).
in file hack.dl [64:1-64:24])_");
{
	Logger logger(R"_(@t-recursive-rule;ids;0;hack.dl [64:1-64:24];ids(a) :- \n   add(_,_,a).;)_",iter, [&](){return rel_new_ids_5220eba57eff354c->size();});
if(!(rel_delta_add_241d705dc2fb23df->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_add_241d705dc2fb23df_op_ctxt,rel_delta_add_241d705dc2fb23df->createContext());
CREATE_OP_CONTEXT(rel_new_ids_5220eba57eff354c_op_ctxt,rel_new_ids_5220eba57eff354c->createContext());
...
signalHandler->setMsg(R"_(ids(a) :- 
   var(_,a).
in file hack.dl [65:1-65:21])_");
{
	Logger logger(R"_(@t-recursive-rule;ids;0;hack.dl [65:1-65:21];ids(a) :- \n   var(_,a).;)_",iter, [&](){return rel_new_ids_5220eba57eff354c->size();});
if(!(rel_delta_var_789565ec55b51a31->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_var_789565ec55b51a31_op_ctxt,rel_delta_var_789565ec55b51a31->createContext());
CREATE_OP_CONTEXT(rel_new_ids_5220eba57eff354c_op_ctxt,rel_new_ids_5220eba57eff354c->createContext());
...
{
	Logger logger(R"_(@t-recursive-relation;leader;hack.dl [60:7-60:13];)_",iter, [&](){return rel_new_leader_99e6be51e44547e4->size();});
signalHandler->setMsg(R"_(leader(a,as(@generator_0, Id)) :- 
   ids(a),
   a = as(@eqfind(as(a, number)), Id),
   @generator_0 = min as(b0, number) : { eql(a,b0),as(b0, number) = as(b0, number) }.
in file hack.dl [66:1-66:118])_");
{
	Logger logger(R"_(@t-recursive-rule;leader;0;hack.dl [66:1-66:118];leader(a,as(@generator_0, Id)) :- \n   ids(a),\n   a = as(@eqfind(as(a, number)), Id),\n   @generator_0 = min as(b0, number) : { eql(a,b0),as(b0, number) = as(b0, number) }.;)_",iter, [&](){return rel_new_leader_99e6be51e44547e4->size();});
if(!(rel_delta_ids_6be932dee18e845d->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_ids_6be932dee18e845d_op_ctxt,rel_delta_ids_6be932dee18e845d->createContext());
...
if( (ramBitCast<RamDomain>(env0[0]) == ramBitCast<RamDomain>(eqfind(((RamSigned)env0[0]))))) {
...
signalHandler->setMsg(R"_(leader(a,b) :- 
   ids(a),
   a != as(@eqfind(as(a, number)), Id),
   leader( _tmp_0,b),
    _tmp_0 = as(@eqfind(as(a, number)), Id).
in file hack.dl [67:1-67:108])_");
{
	Logger logger(R"_(@t-recursive-rule;leader;0;hack.dl [67:1-67:108];leader(a,b) :- \n   ids(a),\n   a != as(@eqfind(as(a, number)), Id),\n   leader( _tmp_0,b),\n    _tmp_0 = as(@eqfind(as(a, number)), Id).;)_",iter, [&](){return rel_new_leader_99e6be51e44547e4->size();});
if(!(rel_delta_ids_6be932dee18e845d->empty()) && !(rel_leader_d565398e1ef92697->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_ids_6be932dee18e845d_op_ctxt,rel_delta_ids_6be932dee18e845d->createContext());
CREATE_OP_CONTEXT(rel_delta_leader_d4159f531d436f83_op_ctxt,rel_delta_leader_d4159f531d436f83->createContext());
CREATE_OP_CONTEXT(rel_new_leader_99e6be51e44547e4_op_ctxt,rel_new_leader_99e6be51e44547e4->createContext());
...
for(const auto& env0 : *rel_delta_ids_6be932dee18e845d) {
if( (ramBitCast<RamDomain>(env0[0]) != ramBitCast<RamDomain>(eqfind(((RamSigned)env0[0]))))) {
auto range = rel_leader_d565398e1ef92697->lowerUpperRange_10(Tuple<RamDomain,2>{{ramBitCast(eqfind(((RamSigned)env0[0]))), ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},Tuple<RamDomain,2>{{ramBitCast(eqfind(((RamSigned)env0[0]))), ramBitCast<RamDomain>(MAX_RAM_SIGNED)}},READ_OP_CONTEXT(rel_leader_d565398e1ef92697_op_ctxt));
for(const auto& env1 : range) {
if( !(rel_leader_d565398e1ef92697->contains(Tuple<RamDomain,2>{{ramBitCast(env0[0]),ramBitCast(env1[1])}},READ_OP_CONTEXT(rel_leader_d565398e1ef92697_op_ctxt))) && !(rel_delta_leader_d4159f531d436f83->contains(Tuple<RamDomain,2>{{ramBitCast(env1[0]),ramBitCast(env1[1])}},READ_OP_CONTEXT(rel_delta_leader_d4159f531d436f83_op_ctxt)))) {
...
in file hack.dl [67:1-67:108])_");
{
	Logger logger(R"_(@t-recursive-rule;leader;1;hack.dl [67:1-67:108];leader(a,b) :- \n   ids(a),\n   a != as(@eqfind(as(a, number)), Id),\n   leader( _tmp_0,b),\n    _tmp_0 = as(@eqfind(as(a, number)), Id).;)_",iter, [&](){return rel_new_leader_99e6be51e44547e4->size();});
if(!(rel_ids_91388717b796d22d->empty()) && !(rel_delta_leader_d4159f531d436f83->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_delta_leader_d4159f531d436f83_op_ctxt,rel_delta_leader_d4159f531d436f83->createContext());
CREATE_OP_CONTEXT(rel_new_leader_99e6be51e44547e4_op_ctxt,rel_new_leader_99e6be51e44547e4->createContext());
CREATE_OP_CONTEXT(rel_ids_91388717b796d22d_op_ctxt,rel_ids_91388717b796d22d->createContext());
...
for(const auto& env0 : *rel_ids_91388717b796d22d) {
if( (ramBitCast<RamDomain>(env0[0]) != ramBitCast<RamDomain>(eqfind(((RamSigned)env0[0]))))) {
auto range = rel_delta_leader_d4159f531d436f83->lowerUpperRange_10(Tuple<RamDomain,2>{{ramBitCast(eqfind(((RamSigned)env0[0]))), ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},Tuple<RamDomain,2>{{ramBitCast(eqfind(((RamSigned)env0[0]))), ramBitCast<RamDomain>(MAX_RAM_SIGNED)}},READ_OP_CONTEXT(rel_delta_leader_d4159f531d436f83_op_ctxt));
for(const auto& env1 : range) {
if( !(rel_leader_d565398e1ef92697->contains(Tuple<RamDomain,2>{{ramBitCast(env0[0]),ramBitCast(env1[1])}},READ_OP_CONTEXT(rel_leader_d565398e1ef92697_op_ctxt)))) {
Tuple<RamDomain,2> tuple{{ramBitCast(env0[0]),ramBitCast(env1[1])}};
rel_new_leader_99e6be51e44547e4->insert(tuple,READ_OP_CONTEXT(rel_new_leader_99e6be51e44547e4_op_ctxt));
}
}
...
in file hack.dl [68:1-70:44])_");
{
	Logger logger(R"_(@t-recursive-rule;leader;0;hack.dl [68:1-70:44];leader(x,y1) <= leader(x,y2) :- \n   eql(y1,y2),\n   y2 = as(@eqfind(as(y1, number)), Id).;)_",iter, [&](){return rel_reject_leader_c121991f9eab43a8->size();});
...
in file hack.dl [68:1-70:44])_");
{
	Logger logger(R"_(@t-recursive-rule;leader;1;hack.dl [68:1-70:44];leader(x,y1) <= leader(x,y2) :- \n   eql(y1,y2),\n   y2 = as(@eqfind(as(y1, number)), Id).;)_",iter, [&](){return rel_reject_leader_c121991f9eab43a8->size();});
...
{
	Logger logger(R"_(@c-recursive-relation;ids;hack.dl [61:7-61:10];)_",iter, [&](){return rel_new_ids_5220eba57eff354c->size();});
[&](){
CREATE_OP_CONTEXT(rel_new_ids_5220eba57eff354c_op_ctxt,rel_new_ids_5220eba57eff354c->createContext());
CREATE_OP_CONTEXT(rel_ids_91388717b796d22d_op_ctxt,rel_ids_91388717b796d22d->createContext());
...
{
	Logger logger(R"_(@c-recursive-relation;leader;hack.dl [60:7-60:13];)_",iter, [&](){return rel_new_leader_99e6be51e44547e4->size();});
[&](){
CREATE_OP_CONTEXT(rel_delta_leader_d4159f531d436f83_op_ctxt,rel_delta_leader_d4159f531d436f83->createContext());
CREATE_OP_CONTEXT(rel_leader_d565398e1ef92697_op_ctxt,rel_leader_d565398e1ef92697->createContext());
...
rel_new_var_7aacd8d7c9129d14->purge();
{
	Logger logger(R"_(@t-relation-savetime;leader;hack.dl [60:7-60:13];savetime;)_",iter, [&](){return rel_leader_d565398e1ef92697->size();});
if (performIO) {
...
{
	Logger logger(R"_(@t-nonrecursive-relation;class;hack.dl [72:7-72:12];)_",iter, [&](){return rel_class_9017e5267072d9ea->size();});
signalHandler->setMsg(R"_(class(b) :- 
   leader(_,b).
in file hack.dl [73:1-73:26])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;class;hack.dl [73:1-73:26];class(b) :- \n   leader(_,b).;)_",iter, [&](){return rel_class_9017e5267072d9ea->size();});
...
{
	Logger logger(R"_(@t-relation-savetime;class;hack.dl [72:7-72:12];savetime;)_",iter, [&](){return rel_class_9017e5267072d9ea->size();});
if (performIO) {
...
{
	Logger logger(R"_(@t-nonrecursive-relation;root;hack.dl [75:7-75:11];)_",iter, [&](){return rel_root_4971aadef59ffd68->size();});
signalHandler->setMsg(R"_(root(1,lx1234) :- 
   ...
   add(lx123,lx4,x1234),
   leader(x1234,lx1234).
in file hack.dl [77:1-84:51])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;root;hack.dl [77:1-84:51];root(1,lx1234) :- \n   var(1,x1),\n   leader(x1,lx1),\n   var(2,x2),\n   leader(x2,lx2),\n   var(3,x3),\n   leader(x3,lx3),\n   var(4,x4),\n   leader(x4,lx4),\n   add(lx1,lx2,x12),\n   leader(x12,lx12),\n   add(lx12,lx3,x123),\n   leader(x123,lx123),\n   add(lx123,lx4,x1234),\n   leader(x1234,lx1234).;)_",iter, [&](){return rel_root_4971aadef59ffd68->size();});
...
signalHandler->setMsg(R"_(root(2,lx4321) :- 
   ...
   add(lx432,lx1,x4321),
   leader(x4321,lx4321).
in file hack.dl [87:1-94:51])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;root;hack.dl [87:1-94:51];root(2,lx4321) :- \n   var(1,x1),\n   leader(x1,lx1),\n   var(2,x2),\n   leader(x2,lx2),\n   var(3,x3),\n   leader(x3,lx3),\n   var(4,x4),\n   leader(x4,lx4),\n   add(lx4,lx3,x43),\n   leader(x43,lx43),\n   add(lx43,lx2,x432),\n   leader(x432,lx432),\n   add(lx432,lx1,x4321),\n   leader(x4321,lx4321).;)_",iter, [&](){return rel_root_4971aadef59ffd68->size();});
if(!(rel_add_48c57da858454f79->empty()) && !(rel_var_09870be8e1a4a08e->empty()) && !(rel_leader_d565398e1ef92697->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_add_48c57da858454f79_op_ctxt,rel_add_48c57da858454f79->createContext());
CREATE_OP_CONTEXT(rel_leader_d565398e1ef92697_op_ctxt,rel_leader_d565398e1ef92697->createContext());
...
{
	Logger logger(R"_(@t-relation-savetime;root;hack.dl [75:7-75:11];savetime;)_",iter, [&](){return rel_root_4971aadef59ffd68->size();});
if (performIO) {
